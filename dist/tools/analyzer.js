...
exports.analyzeProfileTool = void 0;
exports.analyzeProfile = analyzeProfile;
const zod_1 = require("zod");
exports.analyzeProfileTool = {
...
const ProfileDataSchema = zod_1.z.object({
    profileData: zod_1.z.object({
        ...
        connections: zod_1.z.number().optional(),
        hasProfilePhoto: zod_1.z.boolean().optional(),
        hasBanner: zod_1.z.boolean().optional(),
        hasFeatured: zod_1.z.boolean().optional()
    })
});
async function analyzeProfile(params) {
...
function calculateProfileScore(profile) {
    ...
    if (profile.headline && profile.headline.length > 150)
        score += 5;
    if (profile.summary && profile.summary.length > 500)
        score += 10;
    if (profile.experience?.some((exp) => exp.description?.length > 100))
        score += 10;
    ...
    if (profile.summary?.includes('contact') || profile.summary?.includes('reach'))
        score += 5;
...
function analyzeSections(profile) {
    return {
        ...
        experience: {
            ...
            quality: profile.experience?.some((e) => e.description) ? 'detailed' : 'needs detail'
        },
        skills: {
...
function analyzeKeywords(profile) {
    const text = [
        profile.headline || '',
        profile.summary || '',
...
function identifyQuickWins(profile) {
    const quickWins = [];
    if (!profile.hasBanner)
        quickWins.push('Add a professional banner image (5 minutes)');
    if (profile.headline && profile.headline.length < 100)
        quickWins.push('Expand headline with keywords (10 minutes)');
    if (!profile.skills || profile.skills.length < 5)
        quickWins.push('Add 10+ relevant skills (15 minutes)');
    if (!profile.summary)
        quickWins.push('Write compelling About section (30 minutes)');
    return quickWins;
}
function compareTobestPractices(profile) {
    return {
        headline: {
...
function estimateImpact(currentScore) {
...
function calculateKeywordDensity(keywords, text) {
...
function identifyMissingKeywords(profile) {
    ...
    return commonKeywords.filter(keyword => !profileText.includes(keyword));
}
